...
class Document:
    ...
    def __init__(self, originName, metadata):
        ...
        self.blankPdf = '/home/anovikov/repos/stamp_data/pdf/blank.pdf'
        self.outPdf = '../output/new.pdf'
    ...
    def setFontArt(self, t):
        ...
        pdfmetrics.registerFont(TTFont('FreeIt', '../vendor/FreeSerifItalic.ttf'))
        ...
        tt = re.sub("â", "-", tt)
        with open("/home/anovikov/repos/stamp_data/data/csv/whitelist_reg.csv") as wlist:
            wl = csv.reader(wlist)
            wl = list(wl)
            white_reg = []
            for n in wl:
                white_reg.append(n[0])
        with open("/home/anovikov/repos/stamp_data/data/csv/whitelist_freeserif.csv") as wtlist:
...
with open('/home/anovikov/repos/stamp_data/data/csv/fileList.csv') as kurz:
    names = csv.reader(kurz, delimiter=',', quotechar='#')
    for fn in names:
        try:
            originName = fn[3]
            newDoc = Document(originName, '/home/anovikov/repos/stamp_data/data/csv/oai_last.csv')
