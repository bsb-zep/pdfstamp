...
with open('oai_10.csv') as zehn:
	namek = csv.reader(zehn, delimiter='|', quotechar='#')
	for fn in namek:
		try:
			#num = '/'+fn[1]+'/'+fn[2]+'/'+fn[3]
			#num1 = fn[3]
			...
			fnum = fn[8]
			snum = fn[9]
			url = "https://www.bohemia-online.de/index.php/bohemia/article/download/"+fnum+"/"+snum
			r = urlopen(url)
			...
			subprocess.call(["python3 pdf2txt.py -p 1 -t xml /home/anovikov/articles/"+fn1+"/public/"+fna+" > koord2.xml"], shell=True)

			Stempel = io.BytesIO()
			Blank =io.BytesIO()
			tree = etree.parse("koord2.xml")
			root = tree.getroot()
			...
			with open('oai_10.csv') as csvfile:
				...
				tempSeite.showPage() 
				tempSeite.save()
				filee = "/home/anovikov/articles/"+fn1+"/public/"+fna
				# Merge zwei PDFs
				urpdf = open(filee, 'rb')
				...
				ennd = "/home/anovikov/articles_stempel/articles/"+fn1+"/public/"+fna
				endFile = open(ennd, 'wb')
				pdfWriter.write(endFile)
				urpdf.close()
				endFile.close()
		except IndexError:
			print ("Fehler: ", fn1, "/public/", fna)
			#print ("\n\nRow Info: ", row)
			continue
